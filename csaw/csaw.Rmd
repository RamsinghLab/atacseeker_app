...
**Note:** This document summarrizes the csaw workflow and borrows heavily from the userguide. 
...
Specify the bam files and create design matrix.
...
The returned object `data` is a `SummarizedExperiment` object. For future reference, `assay` can be used to obtain the matrix of counts. The coordinates of each window are stored in the `rowRanges`. The total number of reads in each library are stored as `totals` in the `colData`. 
